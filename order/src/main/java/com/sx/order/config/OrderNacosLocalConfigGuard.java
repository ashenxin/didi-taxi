...
public final class OrderNacosLocalConfigGuard
        implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {

    private static final String LOCAL_PROFILE = "local";
    private static final String CONFIG_LOADED_MARKER = "order.nacos.config-loaded";
    ...
    static void verify(Environment environment) {
        ...
        if (!remoteConfigLoaded) {
            throw new IllegalStateException(
                    "Required Nacos config DIDI_TAXI/order-service-local.yml was not loaded"
            );
        }
    }
}
