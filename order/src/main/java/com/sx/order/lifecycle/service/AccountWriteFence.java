...
public class AccountWriteFence {
    ...
    public void lockAndRequireAllowed(long customerId, OrderWriteAction action) {
        ...
        if (projection == null) {
            metrics.writeFence(action.name(), OrderLifecycleMetrics.WriteFenceDecision.UNKNOWN);
        ...
        if (projection.getBusinessStatus() == null || projection.getBusinessStatus() != 0
            ...
            metrics.writeFence(action.name(), OrderLifecycleMetrics.WriteFenceDecision.BLOCKED);
            throw new AccountLifecycleBlockedException("账户当前状态不允许执行" + action);
        }
        metrics.writeFence(action.name(), OrderLifecycleMetrics.WriteFenceDecision.ALLOW);
    }
}
