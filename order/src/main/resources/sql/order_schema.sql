...
CREATE TABLE IF NOT EXISTS `order_idempotent_record` (
    ...
    `action_type` VARCHAR(64) NOT NULL COMMENT '动作类型：创建、乘客取消及司机接单/拒单/取消/到达/开始/完单',
    `passenger_id` BIGINT NOT NULL COMMENT '订单关联乘客ID',
    `order_no` VARCHAR(64) NULL COMMENT '动作关联订单号',
    `status` VARCHAR(32) NOT NULL COMMENT 'PROCESSING/SUCCESS/FAILED',
    `request_hash` VARCHAR(64) NOT NULL COMMENT '请求关键字段 SHA-256',
    `response_snapshot` JSON NULL COMMENT '成功响应快照，如 orderNo 或 replayed',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
