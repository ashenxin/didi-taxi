...
- **签名密钥（结构隔离）**：乘客端使用 **`JWT_SECRET_APP`**（`app.jwt.secret`），网关验签使用 `gateway.jwt.secret-app`，两者必须一致；建议与 admin/driver 端密钥不同以防跨端 token 复用。
...
同时建议开启网关 `aud` 校验（`gateway.jwt.audience-check-enabled=true`）作为防御纵深，以防止跨端误用 token：
...
- 网关：`gateway.jwt.secret-app` 与 `audience-app`
- BFF：`app.jwt.secret`（建议环境变量 `JWT_SECRET_APP`）、`app.jwt.expiration-seconds`
