...
本文档描述当前已落地的 **Spring Cloud Gateway 统一 API 网关**职责边界、路由与跨域策略、JWT 分层校验及与三个 BFF、核心服务的关系，供实现与评审对齐。
...
| 乘客/司机 WebSocket 握手放行与 Upgrade 转发 | 多实例 Sticky、Redis Pub/Sub 或 MQ 广播 |
...
- 网关与三个 BFF **共用同一套签名算法**（如 HMAC / 非对称），保证验签结论一致。
- **推荐三端密钥隔离**：按路径前缀分别使用 Admin/App/Driver 不同密钥（或不同的非对称 keypair），实现“验签级隔离”，降低跨端 token 复用风险（细节见《网关服务_技术.md》§6.2）。
...
- 乘客 `/app/ws/**`、司机 `/driver/ws/**` 已经由网关转发 WebSocket Upgrade；握手在网关白名单放行后由 BFF 校验短期小票。
- 当前只验收单实例会话；多实例 Sticky、Redis Pub/Sub 或 MQ 广播本阶段不开发。

---

## 8. 技术栈与实现边界

- **Spring Cloud Gateway**（WebFlux / Netty），与父工程当前的 **Spring Boot 3.5.16**、**Spring Cloud 2025.0.3**（最终以根 `pom.xml` 为准）对齐，单独 `gateway` 模块纳入多模块工程。
...
| 2026-09-16 | §8：同步当前 Boot 3.5.16、Cloud 2025.0.3 技术基线 |
