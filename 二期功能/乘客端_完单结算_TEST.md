...
> 接口说明：[完单结算 API](乘客端_完单结算_API.md)；技术实现：[完单结算 TECH](乘客端_完单结算_TECH.md)
>
> 当前为已实现的 MVP 本地模拟闭环；未接入真实支付宝或微信金融渠道，司机/车队分成也未实现。

## 边界

路线距离、预计时长和实际计费时长全部来自稳定本地模拟数据；不调用外部地图 API。支付宝/微信仅为逻辑渠道，收银台也为本地模拟。`wallet.payment.mock.enabled`（环境变量 `WALLET_PAYMENT_MOCK_ENABLED`）只允许在 `local/dev/test` 开启，禁止在其他环境绕过启动校验。
...
6. 重新 GET settlement，并检查 `trip_order_settlement` 的最终账单和 `wallet_payment_order` 的每次尝试。优惠券在支付失败时仍保持锁定；支付明确失败后不进行任何后台自动重试（包括定时重试）。
