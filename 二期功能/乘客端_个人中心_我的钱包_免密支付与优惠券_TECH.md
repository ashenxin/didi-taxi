...
> 实现核对（2026-09-16）：`driver-api.finish -> order.finish` 后的本地 mock 结算编排已经贯通，包括冻结计价、最优券锁定、mock 免密/主动支付、核销、结算查询和未结清订单拦截。真实支付签名与回调、退款、对账和司机/车队分成仍未实现；当前支付失败后优惠券保持 `LOCKED`，且不创建后台定时自动重扣任务。
...
10. 支付失败：
   ...
   - 结算进入 `PAYMENT_REQUIRED`
   - 优惠券保持 `LOCKED`，等待乘客主动支付；不安排后台自动重扣。
