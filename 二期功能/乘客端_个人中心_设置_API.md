...
- 注销前必须校验当前账号没有进行中订单、没有未结清订单结算、没有 `LOCKED` 优惠券。
- 注销账号采用逻辑删除：`customer.is_deleted=1`。
- 注销成功后递增 token version，当前登录态立即失效。
- 注销后同手机号允许重新注册，新账号使用新的 `customer.id`，不继承旧订单；旧账号 `UNUSED` 券作废、福利积分清零，手机号哈希领取记录保留用于防重复领取。
...
| 401 | 未登录、token 失效、验证码错误或过期 | `未授权，请重新登录` / `验证码错误或已过期` |
| 404 | 当前账号不存在或已注销 | `账号不存在或已注销` |
| 409 | 存在进行中订单、未结清订单或锁定优惠券 | 按具体风险提示先完成/支付订单或释放锁定券 |
| 502 | order/calculate 注销前检查不可用 | `服务暂时不可用，请稍后重试` |
