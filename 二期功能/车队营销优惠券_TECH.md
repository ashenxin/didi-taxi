...
> 当前实现核对（2026-09-16）：模板、领券、最优券计算、锁定、核销及完单结算主链路已经落地。账单固化后的支付失败或取消不会释放券；真实退款、退款退券和对账仍未实现。
...
只有结算金额固化失败等尚未形成有效账单的补偿场景才释放：

```text
LOCKED -> UNUSED
```

账单已经固化后，支付失败或取消时继续保持 `LOCKED`，由乘客完成原账单支付；不得释放后让同一优惠金额失去对应券状态。
...
7. 支付成功后核销券，更新结算支付状态。
8. 支付失败或取消后保持券为 `LOCKED`，结算进入 `PAYMENT_REQUIRED`，不安排后台自动重扣。
...
- 扫描长时间 `LOCKED` 的券并与权威结算状态对账；未结清账单不得仅因时间经过而释放。
- 扫描支付成功但券未核销，以及账单固化失败但补偿释放未完成的异常订单。
- 输出优惠券发放、领取、使用、优惠总额统计。

## 11. 落地状态

以下能力已在车队营销优惠券和完单结算主链路中补齐；本节保留为实现核对清单：
