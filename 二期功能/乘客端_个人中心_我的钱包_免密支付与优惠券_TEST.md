...
> 实现状态（2026-09-16）：T-WALLET-01～19 与 T-WALLET-20A 所对应的本地 mock 主链路已贯通，并由钱包、计价、订单及完单结算专项自动化测试覆盖；T-WALLET-20 应按当前 mock 支付结果消费验证幂等，不代表真实渠道回调已经接入。真实支付回调、退款和对账仍不在当前实现范围。
...
- **预期**
  ...
  - 优惠券继续保持 `LOCKED`，账单优惠金额不变。
  - 不写入支付失败导致的 `RELEASE` 流水，也不安排后台自动重扣；乘客可使用新的支付意图继续支付。
...
### T-WALLET-20 支付结果重复消费

- **步骤**
  1. 在当前 mock 范围内，对同一支付单重复消费成功结果。
- **预期**
  - 支付单保持 `SUCCESS`。
  - 订单结算表保持已支付。
  - 优惠券只核销一次。
...
> 真实渠道回调的签名验签、通知重放和渠道对账需在真实支付宝/微信接入后另行补充，不能用本用例宣称已经通过。
