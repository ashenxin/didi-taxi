...
/** 校验 map-service 内部接口的 X-Internal-Service-Token，只保护 /api/v1/internal/**。 */
@Component
@EnableConfigurationProperties(MapInternalAuthProperties.class)
@Slf4j
public class MapInternalAuthFilter extends OncePerRequestFilter {
    ...
    public MapInternalAuthFilter(MapInternalAuthProperties properties, ObjectMapper objectMapper) {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return !INTERNAL_ROOT.matches(path) && !INTERNAL_DESCENDANTS.matches(path);
    }
    ...
    private void reject(HttpServletRequest request, HttpServletResponse response, int status, String message)
            throws IOException {
        log.warn("map internal authentication rejected uri={} requestId={} status={}",
