...
### T-DISPATCH-25A 端侧订单写操作幂等重放

分别使用固定 `Idempotency-Key` 完成乘客取消，以及司机接单、拒单、到达前取消、到达、开始、完单，并模拟第一次响应丢失后使用相同 key 和请求内容重试。

预期：第一次返回 `replayed=false`，重试返回 `replayed=true`；订单状态迁移、订单事件、司机乘客隔离、Outbox、结算记录及结算任务均只执行一次；BFF 重放成功后仍触发乘客刷新通知。接单重放在 capacity 资格校验前短路。相同 key 修改关键请求内容或跨动作复用返回 `409`；缺失、空白或超过 128 字符的 key 返回 `400`。完单请求中的旧版里程、时长和金额字段不参与结算，也不改变幂等意图。
