...
public final class AdminApiNacosLocalConfigGuard
        implements ApplicationListener<ApplicationEnvironmentPreparedEvent> {

    private static final String LOCAL_PROFILE = "local";
    private static final String CONFIG_LOADED_MARKER = "admin.nacos.config-loaded";
    ...
    static void verify(Environment environment) {
        ...
        if (!remoteConfigLoaded) {
            throw new IllegalStateException(
                    "Required Nacos config DIDI_TAXI/admin-api-local.yml was not loaded"
            );
        }
    }
}
