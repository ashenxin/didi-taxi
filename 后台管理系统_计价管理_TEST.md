...
本文覆盖 fare rule 管理、基础估价、车队营销优惠券模板、权限数据域和优惠计算。乘客领券风控见《二期功能/乘客端_券包与登录领券_TEST.md》；完单锁券/支付编排见钱包 TEST，且当前尚未端到端贯通。
...
注意：内部结算、锁券、mock 支付接口已存在，但司机 finish 后自动编排尚未贯通。本用例需通过明确调用内部编排步骤验证，不能把“表结构存在”记为端到端通过。

### T-ADMIN-COUPON-17 支付失败/退款边界

当前可验证支付失败释放券内部动作；真实退款流程未实现，不执行“退款闭环已通过”结论。
