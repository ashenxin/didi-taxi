...
结论：本次（2026-07-17）MVP 登录、下单、异步派单、确认窗、接单和行程推进主路径通过；当时尚未把“司机完单后的自动结算/扣款/核券编排”判为通过。

> 后续状态（2026-09-16）：完单后的本地 mock 结算、扣款、核券、主动支付和未结清订单拦截已经贯通，不再作为 P0 未完成项。当前剩余范围是真实支付渠道、退款、对账和司机/车队分成，详见[二期完单结算文档](二期功能/乘客端_完单结算_PRD.md)。
