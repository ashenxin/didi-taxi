...
1. ~~**迟滞匹配定时兜底**~~（已实现：`LateDispatchScheduler` + order `pending-dispatch-all`）。  
2. ~~**改派 / 重开 offer 调度**~~（已实现：`OfferRescheduleScheduler`、`TripOrderWriteService.reassign`、内部 `assigned-awaiting-reschedule`）。  
3. **司机 WS + 网关**（司机端优先于乘客端）。  
4. ~~**心跳 GEO / 登出删池**~~（登出已联动运力删 GEO；心跳续期仍属分期）。  
