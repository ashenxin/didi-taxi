...
本地阶段明确把业务运行所需的秘密值写入 Nacos，包括数据库密码、Redis 密码、内部调用 Token 和 XXL-JOB Token。后续迁移其他服务时，JWT 密钥、第三方访问密钥（例如 `map.amap.key`）也遵循同一策略。Git 中的本地业务配置文件不再保存这些值。

唯一的引导例外是 Nacos 客户端自身的登录凭据：`NACOS_USERNAME` 和 `NACOS_PASSWORD` 继续由进程环境传入。

Nacos 配置中心提供认证、命名空间和权限隔离，但它不是专门的密钥管理系统。本策略只适用于当前受控的本地环境；设计生产环境时必须重新评估最小权限、配置读取审计、备份暴露风险，以及是否改用专门的密钥管理服务或 Nacos 配置加密插件。
...
- 本地配置文件不再含已迁移的运行配置。
- 本地业务配置文件不再含数据库密码、Token 等已迁移秘密值；Nacos 登录凭据仍由环境变量提供。
