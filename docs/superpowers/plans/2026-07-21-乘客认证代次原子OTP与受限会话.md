...
public final class OtpKeyFactory {
    ...
    // required* 方法必须拒绝空手机号、非正 customerId/version 和多余字段。
}
```
...
在 `passenger/pom.xml` 增加 BOM 管理版本的 `org.testcontainers:junit-jupiter` test-scope 依赖，并以 `GenericContainer<>("redis:7.2-alpine").withExposedPorts(6379)` 运行真实 Redis 并发用例：同一 Key 启动 20 个线程同时 `consume`，断言恰好一个 `CONSUMED`、其余为 `MISSING`；不允许用 mock 冒充并发原子性。
...
预期：PASS；并发测试报告恰好一次 `CONSUMED`。

- [ ] **步骤 9：提交任务 1**

```bash
git add passenger/src/main passenger/src/test passenger/pom.xml
