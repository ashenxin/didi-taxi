...
> 执行状态（2026-07-27）：P2 已完成，并已被 P7 公开生命周期入口使用。本文未勾选的
> 复选框是原始逐步实施脚本，不再代表当前待办；当前状态以
> `docs/plans/2026-07-22-乘客生命周期P2-认证代次原子OTP与受限会话执行计划.md`
> 为准。
...
最终收口采用未公开的 `PassengerLifecycleOrchestrator`：核心成功结果显式携带
`customerId/newAuthEpoch/requireLogin/revocationReason`，编排器校验操作确实完成后再同步推进本节点 WS generation
栅栏；失败或不完整响应不关闭会话。未来新增公开换号/注销入口必须复用该编排器，不得把撤销代码重新分散到
Controller 或各业务 Service。P2 不新增 lifecycle Controller，跨实例可靠关闭仍留给 P6。
