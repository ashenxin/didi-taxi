...
> `driver-api` 负责 **订单侧批量拒指派**、**已接未到订单释单改派**、**运力下线**、**JWT 作废**；`capacity` 内网接口不负责 JWT 作废。
...
2. **自动释放已接未到订单**
   - 目标范围：司机名下状态为 **`ACCEPTED`** 且尚未到达上车点的订单。
   - 目标动作：复用司机到达前取消链路（`driver/cancel` 等价能力），将订单 **`ACCEPTED → CREATED`**，清空司机与确认窗口信息，写司机释放/到达前取消类事件，并再次投递派单 Outbox。
   - **语义**：这是司机退出导致的 **释放改派 / 释单**，乘客侧进入“正在重新派单”；**不是**乘客登出那种 **`→ CANCELLED`** 终态。
   - 单笔释单失败应记录告警；推荐不阻断后续运力下线与 token 作废，具体失败补偿在实现方案中细化。
...
**状态边界**：`ARRIVED / STARTED / FINISHED` 等到达后或行程中订单，登出不自动取消、不自动释单；只处理登录态和听单状态。
...
| 日期 | 说明 |
|------|------|
| 2026-06-11 | 登出 §7：补充 **`ACCEPTED` 已接未到自动释单改派**；明确不是乘客侧 `CANCELLED` |
