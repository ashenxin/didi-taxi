...
本文记录 **司机池** 使用 **司机级 Presence + 心跳** 的原因、**司机与上车点匹配距离上限（3km）**、**下线/登出** 时对 Redis 的处理要求、**迟滞匹配** 的推荐触发方式（**入池为主 + 定时兜底**）、**listAssigned** 场景下 **多乘客命中同一司机时仅一单可生效** 的规则，以及 **下线后待接单列表** 的两种产品策略（**当前倾向策略 A**），供实现与评审时对照。
...
仅「听单时写入一次、永不续期」时，会出现 **僵尸条目** 与 **位置失真**。因此需要 **司机级 Presence** 与 **心跳（或定时位置上报）**。

| 场景 | 无 Presence/心跳的风险 | Presence/心跳的应对 |
|------|-------------------|----------------|
| App 强杀、断网、崩溃，未调用下线接口 | Redis 中仍存在该司机，被错误匹配 | 心跳停止后 Presence 过期，清理任务剔除 |
| 长时间听单，车辆位置已变 | 仍按旧坐标做「最近」，接驾距离失真 | 心跳携带当前坐标，顺带 `GEOADD` 更新位置并续 Presence |
| 服务端与客户端脱节（DB 已非听单，Redis 未删） | 仍被搜到为候选 | 下线/登出主动删池 + Presence 过期清理兜底 |

**推荐组合（概念）**：

- **心跳**：客户端按固定间隔上报（或仅在显著移动时上报），服务端更新 **司机池坐标** 并刷新司机级 **Presence**。
- **Presence 过期清理**：超过超时时间无续期则从司机池移除，避免长期残留。

当前实现：

- 司机 H5 听单期间约每 **15 秒**调用 `POST /driver/api/v1/drivers/{driverId}/heartbeat`；有定位时更新 GEO，无定位时仍续 Presence。
- capacity 使用 `tx:driver:presence:{cityCode}` ZSET，member 为 `driverId`、score 为最后心跳时间戳。
- `capacity.dispatch.driver-heartbeat-timeout-seconds` 默认 **60 秒**；匹配阶段直接过滤超时 Presence。
- XXL Handler `capacityDriverPresenceCleanup` 建议每 **30 秒**执行，清理超时 Presence/GEO，并将仍为听单中的司机更新为下线。
- `tx:driver:geo:{cityCode}` 是城市共享 GEO key，**不再设置整个 key 的 TTL**，避免任一司机续期导致全部失联司机永久残留。
...
- **司机池 GEO / Presence 的写入与删除**：业务入口在 `driver-api` 听单、心跳、下线相关接口，通过 Feign 调用 `capacity`，由 capacity 统一执行 `GEOADD` / `ZREM` 与 Presence ZSET 更新。
...
| 文档 | 分工 |
|------|------|
| **本文** | **司机池 / 订单池** 语义、**3km**、**Presence/心跳**、**下线删池**、**迟滞匹配**、**多单一司机**、**listAssigned 策略 A**；**§8 流程图** 以本文为准。 |
| 《`乘客司机端_最小闭环接口调用文档.md`》 | 乘客 **接口**、**H5 调用顺序**、**闭环**；**§1.1b** 引用本文，**不**重复粘贴流程图。 |
| 《`司机端_上线听单与接单设计.md`》 | **状态机**、**offer**、**派单调度扫描（order/capacity 默认约 5s 级；迟滞兜底默认 15s）**、**WS**、**§2.5 多笔待确认**；与本文 **§7** 互引。 |
| 《`司机端_登录注册_TECH.md`》《`司机端_登录注册_API.md`》 | **登出 / JWT**；司机池删池指向本文 **§3**。 |
