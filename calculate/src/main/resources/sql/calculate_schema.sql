...
CREATE TABLE IF NOT EXISTS `benefit_points_flow` (
    ...
    `biz_id` VARCHAR(160) NOT NULL COMMENT '业务ID：签到记录ID或生命周期operationNo:stepCode',
...
-- =============================================================================
-- P4：乘客账户生命周期 Calculate 本地投影与参与者幂等结果
-- =============================================================================
CREATE TABLE IF NOT EXISTS `calculate_account_lifecycle_event_inbox` (
    ...
    `lifecycle_version` BIGINT NOT NULL COMMENT '生命周期版本',
    `request_hash` CHAR(64) NOT NULL COMMENT '事件请求摘要',
    `created_at` DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (`source_event_id`),
    KEY `idx_calculate_lifecycle_event_customer` (`customer_id`, `lifecycle_version`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Calculate 生命周期投影事件永久去重记录';

CREATE TABLE IF NOT EXISTS `calculate_account_lifecycle_projection` (
    `customer_id` BIGINT NOT NULL COMMENT '乘客 ID',
    `business_status` INT NOT NULL COMMENT '乘客业务状态快照',
    ...
    `operation_no` VARCHAR(64) NULL COMMENT '当前生命周期操作号',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='乘客账户生命周期 Calculate 本地投影';
...
CREATE TABLE IF NOT EXISTS `calculate_lifecycle_participant_inbox` (
    ...
    `request_hash` CHAR(64) NOT NULL COMMENT '命令请求摘要',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci COMMENT='Calculate 生命周期参与者幂等结果；BLOCKED终检允许在同一栅栏上刷新';
